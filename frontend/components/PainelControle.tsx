...
import {
  BotInfo, BotParams,
  fetchBotInfo, fetchBotLogs, iniciarBot, pararBot,
} from "@/lib/api";
...
export default function PainelControle() {
  ...
  return (
    <>
      {/* Botão fixo para abrir o painel */}
      <button
      ...
      >
          ...
          <div className="flex items-center gap-3">
            <span
              className={`w-2.5 h-2.5 rounded-full ${
                rodando ? "bg-green-400 animate-pulse" : "bg-gray-500"
              }`}
            />
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-6">
          ...
          <section className="space-y-2">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wide">
              Terminal — últimas {logs.length} linhas
            </h3>
            <div className="bg-gray-900 rounded-xl p-3 h-80 overflow-y-auto font-mono text-xs text-gray-300 space-y-0.5 border border-gray-800">
              {logs.length === 0 ? (
                <p className="text-gray-600">Sem logs. Inicie o bot para ver a saída aqui.</p>
              ) : (
                logs.map((linha, i) => (
                  <div key={i} className={`leading-relaxed ${
                    linha.includes("COMPRA") ? "text-green-400" :
                    linha.includes("VENDA") || linha.includes("Take-Profit") || linha.includes("Stop") ? "text-red-400" :
                    linha.includes("Erro") || linha.includes("erro") ? "text-yellow-400" :
                    "text-gray-400"
                  }`}>
                    {linha || "\u00a0"}
                  </div>
            ...
            <p className="text-xs text-gray-600">Atualiza a cada 10 segundos</p>
          </section>
        </div>
      </div>
    </>
